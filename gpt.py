...
def invoke(prompt: str, T: float = 1.0, gpt4: bool = False) -> str:
    model = ChatOpenAI(model_name=["gpt-3.5-turbo-0125", "gpt-4-0125-preview"][gpt4], temperature=T) # type: ignore
...
async def batch_invoke(prompts: list[str], T: float = 1.0, gpt4: bool = False) -> list[str]:
    model = ChatOpenAI(model_name=["gpt-3.5-turbo", "gpt-4-turbo-preview"][gpt4], temperature=T) # type: ignore
